/**
 * Components - Navigations Node
 */

/* Imports */

import type { NavigationsNodeObj, NavigationIs, NavigationArgs } from './NavigationsNodeTypes'
 ...
 *
 * @private
 * @type {import('./NavigationsNodeTypes').NavigationIs}
 */
 ...
 *
 * @type {import('./NavigationsNodeTypes').NavigationsNodeObj}
 */
const NavigationsNode: NavigationsNodeObj = (args) => {
  /* Args */
      ...
      'main',
      {
        ...
        filterAfterLink: ({ item, index, depth, output }) => {
          ...
          if (_isDropdown(item, depth)) {
            ...
            output.html += `
                <button class="o-collapsible__toggle t-current l-px-4xs l-py-5xs no-js-none" type="button" aria-controls="${collapsibleId}" aria-expanded="false" aria-label="${title} submenu">
        ...
        filterAfterLinkText: ({ item, depth, output }) => {
          ...
          if (_isButtonDropdown(item, depth)) {
            output.html += `
              ...
              ${CaretSvgNode('down', 'l-flex l-wd-3xs l-ht-3xs e-trans no-js-none')}
            `
          }
        }
      }
    ...
    social: nav.getOutput(
      'social',
      {
        ...
        filterAfterLinkText: ({ item, output }) => {
          ...
          const size = t === 'email' || t === 'link' ? 'xs' : '2xs'

          output.html += '</span>'
          output.html += SocialSvgNode(t, `l-flex l-wd-${size} l-ht-${size}`)
        }
      }
    )
  }
  ...
  if (itemWidths.length !== 0 && itemMidIndex !== 0) {
    ...
    configNodeVars.navHalf = Math.max(firstTotal, secondTotal)
  }
 ...
 *
 * @param {import('./NavigationsNodeTypes').NavigationArgs} args
 * @return {string}
 */
const NavigationNode = (args: NavigationArgs): string => {
  /* Args must be object */
...
export { NavigationsNode, NavigationNode }
